		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				if (xmppManager != null) {
					if (!xmppManager.isAuthenticated()) { 
						try {
							synchronized (xmppManager) {
								Log.e(LOGTAG,"wait for authenticated……");
								xmppManager.wait();
							}
						} catch (InterruptedException e) { 
							e.printStackTrace();
					...
					Log.e(LOGTAG,"authenticated already. send SetAliasIQ now……");
	...
	public void setTags(final List<String> tagsList){
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				NotificationService notificationService = NotificationService
						.getNotificationService();
				XmppManager xmppManage = notificationService.getXmppManager();
				if (xmppManage != null) {
					if (!xmppManage.isAuthenticated()) {
						try {
							synchronized (xmppManage) {
								Log.e(LOGTAG, "wait for authenticated..");
								xmppManage.wait();
							}
						} catch (InterruptedException e) {
					...
					Log.e(LOGTAG, "authenticated already send SetTagsIQ now...");
					SetTagsIQ iq = new SetTagsIQ();
					iq.setType(IQ.Type.SET);
					iq.setUsername(username);
					iq.setTagList(tagsList);
					xmppManage.getConnection().sendPacket(iq);
				}
			}
		}).start();
	}
