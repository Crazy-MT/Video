...
public class DirectorMovieDao {
	
	public void addDirector(DirectorMovie director) throws SQLException{
		
		Connection conn  = DBUtil.getConnection();
		int movie_id = director.getMovie_id();
		String sql = "select *from directormovie where movie_id=? ";
		...
		if(rawCount == 0){
			sql = "insert into directormovie (name , movie_id) values(?,?)";
